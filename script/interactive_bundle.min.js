...
/*! Overthrow. An overflow:auto polyfill for responsive design. (c) 2012: Scott Jehl, Filament Group, Inc. http://filamentgroup.github.com/Overthrow/license.txt */
(function(k,b,e){if(b===e){return}b.scrollIndicatorClassName="overthrow";var j=k.document,d=j.documentElement,h=b.support==="native",m=b.canBeFilledWithPoly,c=b.configure,i=b.set,a=b.forget,g=b.scrollIndicatorClassName;b.closest=function(o,n){return !n&&o.className&&o.className.indexOf(g)>-1&&o||b.closest(o.parentNode)};var f=false;b.set=function(){i();if(f||h||!m){return}k.overthrow.addClass();f=true;b.support="polyfilled";b.forget=function(){a();f=false;if(j.removeEventListener){j.removeEventListener("touchstart",o,false)}};var q,u=[],n=[],t,w,v=function(){u=[];t=null},r=function(){n=[];w=null},s,p=function(A){s=q.querySelectorAll("textarea, input");for(var z=0,y=s.length;z<y;z++){s[z].style.pointerEvents=A}},x=function(z,A){if(j.createEvent){var B=(!A||A===e)&&q.parentNode||q.touchchild||q,y;if(B!==q){y=j.createEvent("HTMLEvents");y.initEvent("touchend",true,true);q.dispatchEvent(y);B.touchchild=q;q=B;B.dispatchEvent(z)}}},o=function(G){if(b.intercept){b.intercept()}v();r();q=b.closest(G.target);if(!q||q===d||G.touches.length>1){return}p("none");var H=G,y=q.scrollTop,C=q.scrollLeft,I=q.offsetHeight,z=q.offsetWidth,D=G.touches[0].pageY,F=G.touches[0].pageX,J=q.scrollHeight,E=q.scrollWidth,A=function(N){var K=y+D-N.touches[0].pageY,L=C+F-N.touches[0].pageX,O=K>=(u.length?u[0]:0),M=L>=(n.length?n[0]:0);if((K>0&&K<J-I)||(L>0&&L<E-z)){N.preventDefault()}else{x(H)}if(t&&O!==t){v()}if(w&&M!==w){r()}t=O;w=M;q.scrollTop=K;q.scrollLeft=L;u.unshift(K);n.unshift(L);if(u.length>3){u.pop()}if(n.length>3){n.pop()}},B=function(K){p("auto");setTimeout(function(){p("none")},450);q.removeEventListener("touchmove",A,false);q.removeEventListener("touchend",B,false)};q.addEventListener("touchmove",A,false);q.addEventListener("touchend",B,false)};j.addEventListener("touchstart",o,false)
}})(this,this.overthrow);
/*! Overthrow. An overflow:auto polyfill for responsive design. (c) 2012: Scott Jehl, Filament Group, Inc. http://filamentgroup.github.com/Overthrow/license.txt */
(function(a,d,c){if(d===c){return}d.easing=function(f,e,h,g){return h*((f=f/g-1)*f*f+1)+e};d.tossing=false;var b;d.toss=function(f,q){d.intercept();var k=0,o=f.scrollLeft,p=f.scrollTop,m={top:"+0",left:"+0",duration:50,easing:d.easing,finished:function(){}},n,e,g=false;if(q){for(var h in m){if(q[h]!==c){m[h]=q[h]}}}if(typeof m.left==="string"){m.left=parseFloat(m.left);n=m.left+o}else{n=m.left;m.left=m.left-o}if(typeof m.top==="string"){m.top=parseFloat(m.top);e=m.top+p}else{e=m.top;m.top=m.top-p}d.tossing=true;b=setInterval(function(){if(k++<m.duration){f.scrollLeft=m.easing(k,o,m.left,m.duration);f.scrollTop=m.easing(k,p,m.top,m.duration)}else{if(n!==f.scrollLeft){f.scrollLeft=n}else{if(g){m.finished()}g=true}if(e!==f.scrollTop){f.scrollTop=e}else{if(g){m.finished()}g=true}d.intercept()}},1);return{top:e,left:n,duration:d.duration,easing:d.easing}};d.intercept=function(){clearInterval(b);d.tossing=false}})(this,this.overthrow);
...
(function(a,b){if(a.overthrow&&a.addEventListener){function c(d){overthrow.intercept();var e=overthrow.closest(d);if(e){overthrow.toss(e,{left:d.offsetLeft-e.offsetLeft,top:d.offsetTop-e.offsetTop})}}a.document.addEventListener("click",function(h){var f=h.target;if(f&&f.className.indexOf("throw")>-1){var g=f.href.split("#")[1],d=a.document.getElementById(g);if(d){h.preventDefault();c(d);a.location.hash=g}}},false)}})(this);
/*!
